...
\begin{itemize}
\item Weekly scrum evaluations. At the end of scrum iteration, we evaluate what we have done and set the target for next week. This keeps the deadlines 'SMART' and manageable .
...
\begin{itemize}
\item Run Python code on an Android application. As the Tor-tunnel functionality is written in Python code, we need to be able to run Python code on an Android device as well. An library exist where you can write Python for Android, but we will still face a challenge when we will try to combine other libraries.
\item As we dependent on third party code, we might lose time to understand or read certain pats of code or documentation as well as link pieces of code that belong to different parties together.
\end{itemize}
